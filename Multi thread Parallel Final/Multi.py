...
def open_user_booking_window(user_name, booking_system, update_seat_label, update_seat_color):
    ...
    def update_seat_status_periodically():
        while True:
    ...
    threading.Thread(target=update_seat_status_periodically, daemon=True).start()
...
if __name__ == "__main__":
    ...
    if user_names:
        ...
        admin_thread = threading.Thread(target=open_admin_dashboard, args=(booking_system,), daemon=True)
        admin_thread.start()

        for user_name in user_names:
            user_thread = threading.Thread(target=open_user_booking_window, args=(user_name, booking_system, open_admin_dashboard, open_admin_dashboard), daemon=True)
            user_thread.start()
